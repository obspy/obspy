...
class SlownessModel(object):
    ...
    # DepthRange objects, containing the top depth and bottom depth.
    fluidLayerDepths = []
    # TODO I think these are pretty slow. Maybe use numpy arrays or
    # something like that?
    PLayers = []
    SLayers = []
    ...
    def findCriticalPoints(self):
        ...
        for layerNum, layer in enumerate(self.vMod.layers):
            ...
            currPLayer = SlownessLayer.create_from_vlayer(currVLayer,
                                                          self.PWAVE)
            ...
            if inFluidZone \
                ...
                currSLayer = SlownessLayer.create_from_vlayer(currVLayer,
                                                              self.SWAVE)
    ...
    def coarseSample(self):
        ...
        for layer in self.vMod.layers:
            ...
            currPLayer = SlownessLayer.create_from_vlayer(origVLayer,
                                                          self.PWAVE)
            self.PLayers.append(currPLayer)
            if self.depthInFluid(origVLayer.topDepth) or (
                ...
                currSLayer = SlownessLayer.create_from_vlayer(origVLayer,
                                                              self.SWAVE)
            self.SLayers.append(currSLayer)
