...
class SeismicPhase(object):
    ...
    def parseName(self, tMod):
        ...
        if currLeg == "p" or currLeg.startswith("P") or currLeg == "K" \
                or currLeg == "k":
            isPWave = True
        ...
        if currLeg.startswith("s") or currLeg.startswith("S"):
            ...
            if tMod.cmbDepth < sdep < tMod.iocbDepth:
                ...
                return
        if currLeg.startswith("P") or currLeg.startswith("S") or \
                (self.expert and any(currLeg.startswith(a)
                                     for a in ("K", "I"))):
        ...
        elif currLeg == "p" or currLeg == "s" or \
                (self.expert and currLeg.startswith("k")):
        ...
        for legNum in range(len(self.legs) - 1):
            ...
            if len(self.branchSeq) > 0 and isPWavePrev != isPWave:
                self.phaseConversion(tMod, int(self.branchSeq[-1]),
                ...
                if any(nextLeg == c for c in ("P", "S", "Pn", "Sn", "END")):
                    if self.endAction == self.TRANSDOWN or \
                        ...
                        self.addToBranch(tMod, self.currBranch,
                                         ...
                                         self.TURN)
                    self.addToBranch(tMod, self.currBranch, 0, isPWave,
                                     self.REFLECTTOP)
                ...
                if currLeg == "Pdiff" or currLeg == "Sdiff":
                    ...
                    if currLeg == "Pg" or currLeg == "Sg":
                        ...
                        if (self.maxRayParam >= tMod.getTauBranch(
                            ...
                            if nextLeg == "END" or nextLeg.startswith("P") \
                                    or nextLeg.startswith("S"):
                                self.addToBranch(tMod, self.currBranch, 0,
                                                 isPWave, self.REFLECTTOP)
                        else:
                ...
                if nextLeg == "P" or nextLeg == "S":
                    if any(prevLeg == p
                        ...
                        self.addToBranch(tMod, self.currBranch,
                                         ...
                                         self.TURN)
                    self.addToBranch(tMod, self.currBranch, tMod.cmbBranch,
                                     isPWave, self.TRANSUP)
                elif nextLeg == "K":
    ...
    def sumBranches(self, tMod):
        ...
        if self.maxRayParamIndex == 0 \
                and self.minRayParamIndex == len(tMod.rayParams) - 1:
            ...
            for rayNum in range(len(self.dist) - 1):
                if searchDist == self.dist[rayNum + 1] and \
                    ...
                    if self.rayParams[rayNum] == self.rayParams[rayNum + 1] \
                        ...
                        # arrival.
                        continue
                    arrivals.append(self.linearInterpArrival(
...
def legPuller(name):
    ...
    if name.endswith("kmps"):
        ...
        while offset < len(name):
            ...
            if any(nchar == c for c in ("K", "k", "I", "i", "J", "p", "s",
                                        "m", "c")):
                legs.append(nchar)
                offset += 1
            elif nchar == "P" or "S":
