...
(With all angles to be given in degrees)
        ...
        action = _get_deprecated_argument_action('--show_isotropic_part',
                                           '--show-isotropic-part',
...
#        group_show = parser_convert.add_argument_group('Appearance')
        ...
        # Deprecated arguments
        action = _get_deprecated_argument_action('--basis_vectors',
                                           '--basis-vectors',
                                           real_action='store_true')
        group_misc.add_argument(
        ...
        action = _get_deprecated_argument_action('--full_sphere', '--full-sphere',
                                           real_action='store_true')
        group_misc.add_argument(
            '--full_sphere', dest='plot_full_sphere', nargs=0,
            action=action, help=SUPPRESS)

        action = _get_deprecated_argument_action('--input_system', '--input-system')
        group_misc.add_argument(
            '--input_system', dest='plot_input_system',
            type=caps, choices=ALLOWED_BASES, default='NED',
            action=action, help=SUPPRESS)

        action = _get_deprecated_argument_action('--lines_only', '--lines-only',
                                           real_action='store_true')
        group_misc.add_argument(
            '--lines_only', dest='plot_only_lines', nargs=0,
            action=action, help=SUPPRESS)

        action = _get_deprecated_argument_action('--output_file', '--output-file')
        group_misc.add_argument(
            '--output_file', dest='plot_outfile', action=action, help=SUPPRESS)

        action = _get_deprecated_argument_action('--pa_system', '--pa-system',
                                           real_action='store_true')
        group_misc.add_argument(
            '--pa_system', dest='plot_pa_plot', nargs=0,
            action=action, help=SUPPRESS)

        action = _get_deprecated_argument_action('--pressure_colour',
                                           '--pressure-colour')
        group_misc.add_argument(
            '--pressure_colour', dest='plot_pressure_colour',
            action=action, help=SUPPRESS)

        action = _get_deprecated_argument_action('--show1fp', '--show-1fp',
                                           real_action='store_true')
        group_misc.add_argument(
