...
formats=(str,str,float,int,int,int,float,int,float,float,float,str,str,str,str,str,str,int,int,str)
...
def main(wfdisc):
    ...
    for line in wfdisc:
        ...
        destname = "%s-%s-%s-%s.ASC" % (os.path.splitext(parts[16])[0], parts[0], parts[1], time.strftime("%Y%m%d-%H%M%S", time.gmtime(parts[2])))
        print "station %s, component %s, %u samples" % (parts[0], parts[1], parts[7])
        print "=> %s ..." % destname
    
        # check if already there
        if os.path.exists(destname):
            print "I won't overwrite existing file \"%s\", skipping..." % os.path.split(destname)[1]
            continue
...
def convert(parts):
    ...
    try:
        ...
        print "error reading binary packed data from \"%s\"" % os.path.split(destname)[1]
        return False
...
def buildheader(parts):
    ...
    headers.append("DELTA: %e" % (1/parts[8]))
    ...
    headers.append("START: %s" % time.strftime("%Y-%b-%d_%H:%M:%S", time.gmtime(parts[2])))
...
if __name__ == '__main__':
    try:
        ...
        all traces referenced by the given wfdisc file will be converted to ascii
        """
    except IOError:
