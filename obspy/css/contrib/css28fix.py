...
"""quick and dirty conversion routine from CSS 2.8 format to Seismic Handler ASCII format
...
timedata = map(int, head[1].split(".")) # 0 -> timestamp, 1-> millisecs
...
try:
    import pylab
    pylab.plot([x*SH["DELTA"] for x in xrange(SH["LENGTH"])], [d*SH["CALIB"] for d in data])
    pylab.show()
except:
