...
def vidale_adapt(stream, noise_thres, fs, flow, fhigh, spoint, stime, etime,adaptive=True):
    """
    ...
    :param adaptive: switch for adaptive window estimation defaults to True
        if set to False the window will be estimated as 3*max(1/(fhigh-flow),1/flow)
    :type adaptive: bool
    ...
    while True:
        ...
        # in order to account for errors in the inst freq estimation
            ...
            adapt = np.maximum(int(3. * fs/(fhigh-flow )),int(3. * fs/flow))
...
def polarization_analysis(stream, win_len, win_frac, frqlow, frqhigh, stime,
                          etime, verbose=False, method="pm", var_noise=0.0,adaptive=True):
    """
