...
class SlownessmodelSimpleInit(SlownessModel):
    ...
    BUT without the call to createSample in the end. Inherit all other
    methods.
    """
    ...
    def __init__(self, vMod, minDeltaP=0.1, maxDeltaP=11, maxDepthInterval=115,
                 maxRangeInterval=2.5 * pi / 180,
                 maxInterpError=0.05, allowInnerCoreS=True,
                 slowness_tolerance=1e-16, pLayers=[],
                 sLayers=[]):
...
class TestSlownessmodelSlowly(unittest.TestCase):
    ...
    def testSlownessModelSlowly(self):
        ...
        # Check values in between by comparing with variable states from
        # Java debugger.
        ...
        # Can't put these into separate test methods because they're not
        # called in order!
        ...
        # Zero thickness layer, but p changes.
        self.assertEqual(self.tm.PLayers[40].topDepth, 410)
        # Same as top of deepest layer.
        self.assertEqual(self.tm.PLayers[305].botP, 4.511284884393324)
        self.assertEqual(self.tm.SLayers[0].topP, 1896.1309523809525)
        # Zero thickness layer, but p changes.
        self.assertEqual(self.tm.SLayers[284].botDepth, 6354.919811136119)
