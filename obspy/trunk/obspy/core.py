...
from obspy.util import Stats, getParser, getStreamParser
import os


__all__ = ['Trace', 'Stream']
...
class StreamBase(object):
    """
    Dummy class for Stream inheritance.
...
class Stream(StreamBase):
    """
    A general seismic stream class containing Traces.
    
    This class automatically searches for parsers. Do not inherit from
    this class.
    
    Only useful for seismic formats that can handle more than one trace in one
    file.
    """
    methods = ['read', 'write', 'check']
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.stats = Stats()
        self._formats = []
        # inherit from all obspy stream classes
        Stream.__bases__ = getStreamParser()
        super(Stream, self).__init__()
        # create parser specific methods
        for base in Stream.__bases__:
            if not hasattr(base, '__format__'):
                continue
            if not isinstance(base.__format__, basestring):
                continue
            ...
            for method in self.methods:
                ...
                if not hasattr(base, method):
                    continue
                setattr(Stream, func_name, getattr(base, method))
    
    def read(self, filename, format=None, **kwargs):
        """
        Reads a stream from a given file.
