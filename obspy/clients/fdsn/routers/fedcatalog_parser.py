...
class FederatedResponse(RoutingResponse):
    ...
    def get_request_fn(self, target_service):
        ...
        """
        fun = {
            "DATASELECTSERVICE": self.submit_waveform_request,
            "STATIONSERVICE": self.submit_station_request
        }
        if not (target_service == 'waveform' or
                target_service == 'STATIONSERVICE'):
            raise ValueError(
                "Expected either waveform or station, but got {0}",
                target_service)
    ...
    def submit_waveform_request(self, output, failed):
        ...
        try:
            ...
            data = client.get_waveforms_bulk(
                bulk=self.text("DATASELECTSERVICE"))
