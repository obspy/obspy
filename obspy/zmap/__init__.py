...
Reading ZMAP is lenient, i.e. zmap will try to import a file even if it doesn't
strictly conform to 10 or 13 column ZMAP. Namely the following deviations from
standard ZMAP format are acceptable:
...
* *Column 11 (Horizontal error):* is stored in
  `origin.origin_uncertainty.horizontal_uncertainty` with
  `preferred_description` set accordingly.
* *Column 12 (Depth error):* is stored in `origin.depth_errors.uncertainty`
* *Column 13 (Magnitude error):* is stored in
