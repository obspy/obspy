...
Still missing:
 ...
 - to_uncert()  (convert ellipse to covariance matrice, needed to
                 convert QuakeML to nordic, for example)
"""
...
class _ellipse:
    def __init__(self, a, b, theta=0, center=(0, 0)):
        ...
        Inputs:
            ...
            theta=azimuth (degrees CW from 0=N (y positive))
            center=x,y coordinates of ellipse center (0,0)
        """
        if a < b:
            print('Semi-major smaller than semi-minor! Switching...')
            self.a = b
            self.b = a
        else:
            self.a = a
    ...
    def __get_tangents(self, pt=(0, 0)):
        ...
        ms = np.roots([self.a**2 - y1**2, 2*y1*x1, self.b**2 - x1**2])
        cs = -(y1 * ms) + x1
    ...
    def subtended_angle(self, pt=(0, 0), debug=False):
