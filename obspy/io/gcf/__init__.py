...
==============================================================================
This module provides read and write support for `GCF<https://www.guralp.com/apps/ok?doc=GCF_Intro>` 
...
>>> st = read("/path/to/20160603_1955n.gcf")  #doctest: +ELLIPSIS
...
Function may raise TypeError on bad arguments (e.g. out of range) and IOError on failure
to read/decode file or errors in data blocks in file (bad blocks may be returned by use
of keyword argument ``errorret`` though, preventing IOError's raised by these)
...
both in ``stats.gcf`` and set by its correspondng keyworde argument, the keyword argument will 
have precedenc. For default values if neither set by ``stats.gcf`` nor by use of appropriate 
keyword arguments refer to :meth:`~obspy.io.gcf.core._read_gcf`
...
The GCF format only supports a restricted set of sampling rates. To check if a sampling
rate is supported function :func:`~obspy.io.gcf.core.compatible_sps` is provided.

The GCF format only supports fractional start of first data sample for sampling rates > 250 Hz.
For lower sampling rates first data must be sampled at whole second. For greater sampling rates
fractional start time is permitted but restricted and smapling rate specific. For info on permitted
fractional start time for a given sampling rate, function :func:`~obspy.io.gcf.core.get_time_denominator`
is provided
...
+---------------------------------------------------+--------------------------------------------------------------------------------------------+
| :func:`~obspy.io.gcf.core.compatible_sps`         | Checks if a sampling rate is compatible with the implemented GCF format                    |
+---------------------------------------------------+--------------------------------------------------------------------------------------------+
| :func:`~obspy.io.gcf.core.get_time_denominator`   | Returns the time fractional offset denominator, d, associated with an input sampling rate. |
+---------------------------------------------------+--------------------------------------------------------------------------------------------+
"""
