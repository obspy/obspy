...
class QuakeMLTestCase(unittest.TestCase):
    ...
    def _compareStrings(self, doc1, doc2):
        ...
        str1 = ''.join([s.strip() for s in
            tostring(obj1).decode("utf-8").splitlines()])
        obj2 = fromstring(doc2)
        str2 = ''.join([s.strip() for s in
            tostring(obj2).decode("utf-8").splitlines()])
        if str1 != str2:
            print
            print(str1)
            print(str2)
    ...
    def test_enums(self):
        ...
        for enum_name, enum_items in \
            compatibility.iteritems(xsd_enum_definitions):
    ...
    def test_preferred_tags(self):
        ...
        # testing ids
        ev = catalog.events[0]
        self.assertEqual('smi:orig2', ev.preferred_origin_id.resource_id)
        self.assertEqual('smi:mag2', ev.preferred_magnitude_id.resource_id)
        self.assertEqual('smi:fm2',
