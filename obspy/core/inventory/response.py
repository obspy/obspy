...
def response_from_respfiles(sensor_resp_file, datalogger_resp_file,
                            frequency=None):
    """
    ...
    with open(sensor_resp_file, 'r') as sf:
        with open(datalogger_resp_file, 'r') as df:
            return response_from_resps(sf.read(), df.read(),
                                       frequency=frequency)


def response_from_resp(resp_data, frequency=None, sr=None):
    """
    ...
    blockettelist = parser.read_resp(resp_data)
    xseedparser = parser.make_xseed(blockettelist)

    resp_stages = list()

    # Stage 1 from sensor
    b53 = xseedparser.blockettes[53][0]
    b58 = next((b for b in xseedparser.blockettes[58]
                if b.stage_sequence_number == 1), None)
    xseedparser._get_abbreviation
    ...
    stage1 = PolesZerosResponseStage(
        ...
        input_units=lookup_unit(xseedparser.abbreviations,
                                b53.stage_signal_input_units),
        output_units=lookup_unit(xseedparser.abbreviations,
    ...
    for b in xseedparser.stations[0]:
...
def response_from_resps(sensor_resp_data, datalogger_resp_data,
