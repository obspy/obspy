...
class ResourceIdentifier(object):
    ...
    To conform to QuakeML definition it has to be of the following general
    form::

        [smi|quakeml]:<authority-id>/<resource-id>[#<local-id>]

    The exact definition of valid characters in each parts is given by this
    regular expression::

        (smi|quakeml):[\w\d][\w\d\-\.\*\(\)_~']{2,}/
        [\w\d\-\.\*\(\)_~'][\w\d\-\.\*\(\)\+\?_~'=,;#/&]*
    ...
    * ``quakeml:google.org/pick/unique_pick_id``
    ...
    >>> ResourceIdentifier('smi:local/2012-04-11--385392')
    ResourceIdentifier(id="smi:local/2012-04-11--385392")
    ...
    >>> ResourceIdentifier(prefix='smi:my_agency')
    ResourceIdentifier(id="smi:my_agency/...")
    ...
    >>> res_id = "smi:obspy.org/event/test"
    ...
    def __init__(self, id=None, prefix="smi:local/",
        ...
        if id is None:
            ...
            regex = r"^(smi|quakeml):[a-zA-Z0-9][\w\d\-\.\*\(\)_~']{2,}/$"
            # disallow setting a non-conforming QuakeML URI prefix
            if re.match(regex, prefix) is None:
                msg = ("Can only set a valid QuakeML URI prefix. See "
                       "documentation of ResourceIdentifier class for "
                       "details.")
                raise ValueError(msg)
            self.fixed = False
            self._prefix = prefix
            ...
            # check for valid QuakeML URI
            regex = r"^(smi|quakeml):[\w\d][\w\d\-\.\*\(\)_~']{2,}/" + \
                    r"[\w\d\-\.\*\(\)_~'][\w\d\-\.\*\(\)\+\?_~'=,;#/&]*$"
            # disallow setting a fixed non-conforming QuakeML URI
            if re.match(regex, id) is None:
                msg = ("Can only set a valid QuakeML URI. See "
                       "documentation of ResourceIdentifier class for "
                       "details.")
                raise ValueError(msg)
            self.fixed = True
    ...
    @deprecated("Only valid QuakeML URIs are allowed for ResourceIdentifier "
                "strings")
    ...
    def id(self, value):
        regex = r"^(smi|quakeml):[\w\d][\w\d\-\.\*\(\)_~']{2,}/" + \
                r"[\w\d\-\.\*\(\)_~'][\w\d\-\.\*\(\)\+\?_~'=,;#/&]*$"
        # disallow setting a non-conforming QuakeML URI
        if re.match(regex, str(value)) is None:
            msg = ("Can only set a valid QuakeML URI as a fixed "
                   "ResourceIdentifier string. See documentation "
                   "of ResourceIdentifier class for details.")
            raise ValueError(msg)
        self.fixed = True
