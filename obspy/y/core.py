...
def __parseTag(fh):
    ...
    # to this tag.
    next_tag = unpack('%si' % (endian), data[4:8])[0]
    ...
    # the same type.
    next_same = unpack('%si' % (endian), data[8:12])[0]
...
def readY(filename, headonly=False, **kwargs):  # @UnusedVariable
    ...
    while True:
        ...
        if tag_type == 1:
            # TAG_STATION_INFO
            # UCHAR Update[8]
            #   This field is only used internally for administrative purposes.
            #   It should always be set to zeroes.
            # UCHAR Station[5] (BLANKPAD)
            #   Station is the five letter SEED format station identification.
            ...
            #   SensorType is some text describing the type of sensor used at
            #   the station.
            # UCHAR DataFormat[7] (ASCIIZ)
            #   DataFormat is some text describing the data format recorded at
            #   the station.
            data = fh.read(next_tag)
            parts = unpack('5s2s3s51s61s31s51s7s', data[8:])
            ...
            # FLOAT Dip
            #   Dip is the dip of the sensor. 90 degrees is defined as vertical
            #   right way up.
            data = fh.read(next_tag)
            parts = unpack('%sffffff' % (endian), data[8:])
            params = AttribDict()
            ...
            params.sens_units = parts[6].rstrip('\x00')
            params.calib_units = parts[7].rstrip('\x00')
            ...
            data = fh.read(next_tag)
            parts = unpack('%sd16s' % (endian), data[8:])
            params = AttribDict()
            params.load_date = parts[0]
            params.key = parts[1].rstrip('\x00')
            ...
            #   This is the format of the data. This should always be “YFILE”.
            # UCHAR FormatVersion[8] (ASCIIZ)
            #   FormatVersion is the version of the format of the data. This
            #   should always be “5.0”
            data = fh.read(next_tag)
            parts = unpack('%sddLlll8s8s' % (endian), data[16:])
            ...
            params.format = parts[6].rstrip('\x00')
            params.format_version = parts[7].rstrip('\x00')
            ...
            data = fh.read(next_tag)
            parts = unpack('260s', data[8:])
            params = AttribDict()
            params.path_name = parts[0].rstrip('\x00')
