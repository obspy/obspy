#---------------------------------------------------------------------
#  Filename: obspy.py
#   Purpose: Python Seismogram Parser
#   Version: n.a.
...
class Parser:
  """General implementation of a seismogram parser class.

  Attributes are right now
  starttime -- jul seconds
  endtime   -- jul seconts
  samprate  -- in hz
  station   -- station name
  dataquality -- if available, else None
  channel   -- channel
  numsamples -- number of samples

  Methods are 
  jul2date  -- jul seconds to readable data
  __str__   -- overloaded to get a nice overfiew
  ...
  def jul2date(julsec):
    ...
    date = time.strftime('%Y,%m,%d,%H:%M:%S',time.gmtime(julsec))
    # msr_time x - longlong(x/1e6) = julsec
    # (x - julsec)/1e3 = msec
