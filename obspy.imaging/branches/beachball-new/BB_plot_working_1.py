...
def plot_bb(M, borderline_points=360, outfile=None, format=None):
    ...
    M = M - diag( array([1./3*trace(M),1./3*trace(M),1./3*trace(M)] ) )

    print '\nreduced M'
    print M

    trace_M = trace(M)
    if abs( trace(M)) < epsilon:
        trace_M = 0 
    ...
    if trace_M > 0:
        ...
        clr = 1
    if (EW1 < 0 and trace(M) >= 0):
        print 'Houston, we have had a problem  - check M !!!!!!'
        raise SystemExit
    EWn = EW2
    EVn = EV2

    chng_basis =  matrix(zeros((3,3)))
    ...
    phi   = (arange(degree_steps)/float(degree_steps) + 1./degree_steps )*2*pi
    ...
    RHS   = -EWs /(EWn * cos(phi)**2 + EWh * sin(phi)**2 )
    alpha = arctan(sqrt(RHS) ) /pi * 180 

    # set both curves
    r_hor             =  sin(alpha /180.*pi)

    H_values          =  sin(phi) *r_hor
    N_values          =  cos(phi) *r_hor

    S_values_positive =  cos(alpha/180.*pi)
    S_values_negative = -cos(alpha/180.*pi)

    #--------------------------------------------------------------------------
    # change basis back to original input reference system

    line_tuple_pos = zeros((3,degree_steps ))
    line_tuple_neg = zeros((3,degree_steps ))

    for ii in arange(degree_steps):
        pos_vec_in_EV_basis = array([H_values[ii],N_values[ii],S_values_positive[ii] ]).transpose()
        neg_vec_in_EV_basis = array([H_values[ii],N_values[ii],S_values_negative[ii] ]).transpose()
        line_tuple_pos[:,ii] = dot(chng_basis, pos_vec_in_EV_basis )
        line_tuple_neg[:,ii] = dot(chng_basis, neg_vec_in_EV_basis )


    all_EV = zeros((3,6))

    EVh_orig     = dot(chng_basis, EVs)
    all_EV[:,0]  =  EVh.transpose() #_orig.transpose()
    EVn_orig     = dot(chng_basis, EVn)
    all_EV[:,1]  = EVn.transpose() # _orig.transpose()
    EVs_orig     = dot(chng_basis, EVh)
    all_EV[:,2]  = EVs.transpose() # _orig.transpose()
    EVh_orig_neg = dot(chng_basis, EVs)
    all_EV[:,3]  = -EVh.transpose() #_orig_neg.transpose()
    EVn_orig_neg = dot(chng_basis, EVn)
    all_EV[:,4]  = -EVn.transpose() # _orig_neg.transpose()
    EVs_orig_neg = dot(chng_basis, EVh)
    ...
    def stereo_vertical(coords,co_system='NED'):
        ...
        stereo_coords = zeros((2,n_points))

        for ll in arange(n_points):
    ...
    #positive curve
    #
    points_pos_in_order = zeros((2,len(pos_points_stereographic[0,:])))

    #in polar coordinates
    #
    r_phi_positive_curve = zeros((len(pos_points_stereographic[0,:]),2))
    for ii in arange(len(pos_points_stereographic[0,:])):
        r_phi_positive_curve[ii,0] = sqrt(pos_points_stereographic[0,ii]**2 + pos_points_stereographic[1,ii]**2 )
        r_phi_positive_curve[ii,1] = arctan2(pos_points_stereographic[0,ii],pos_points_stereographic[1,ii])% (2*pi)
        
    #find index with highest r
    largest_r_idx = argmax(r_phi_positive_curve[:,0])

    #check, if perhaps more values with same r - if so, take point with lowest phi
    other_idces = list(where(r_phi_positive_curve[:,0]==r_phi_positive_curve[largest_r_idx,0]))
    if len(other_idces) > 1:
        best_idx      = argmin(r_phi_positive_curve[other_idces,1])
    ...
    if keep_direction:
        ...
        for jj in arange(len(pos_points_stereographic[0,:])):
    ...
    angle_point_1 = (arctan2(points_pos_in_order[0,0],points_pos_in_order[1,0])%(2*pi))
    angle_point_2 = (arctan2(points_pos_in_order[0,1],points_pos_in_order[1,1])%(2*pi))
    angle_point_3 = (arctan2(points_pos_in_order[0,2],points_pos_in_order[1,2])%(2*pi))
    ...
    if abs( angle_diff_12) > abs( angle_diff_23):
        r_old = sqrt(points_pos_in_order[0,0]**2 + points_pos_in_order[1,0]**2)
        new_angle = (angle_point_2 - angle_diff_23)%(2*pi)
        points_pos_in_order[0,0] = r_old * sin(new_angle)
        points_pos_in_order[1,0] = r_old * cos(new_angle)
    ...
    r_last_point          = sqrt(points_pos_in_order[0,-1]**2 + points_pos_in_order[1,-1]**2   )
    dist_last_first_point = sqrt( (points_pos_in_order[0,-1] - points_pos_in_order[0,0])**2 + (points_pos_in_order[1,-1] - points_pos_in_order[1,0] )**2 )
    ...
    if dist_last_first_point > (2 - r_last_point):
        ...
        phi_end   = arctan2(points_pos_in_order[0,-1],points_pos_in_order[1,-1])% (2*pi)
        phi_start = arctan2(points_pos_in_order[0,0],points_pos_in_order[1,0])% (2*pi)


        #add one point on the edge every 2pi/360 and two on the direct projections 

        phi_end_larger   = sign(phi_end-phi_start)
        angle_smaller_pi = sign( pi - abs(phi_end - phi_start) )
        ...
        n_edgepoints = int(openangle/pi*180 * n_curve_points/360) + 1
        ...
        if go_ccw:
            ...
            for kk in arange(n_edgepoints):
                current_phi = phi_end - kk*openangle/(n_edgepoints-1)
                points_pos_in_order.append([2*sin(current_phi), 2*cos(current_phi) ]   )
            points_pos_in_order = array(points_pos_in_order).transpose()
        else:
            points_pos_in_order = list(points_pos_in_order.transpose())
            for kk in arange(n_edgepoints):
                current_phi = phi_end + kk*openangle/(n_edgepoints-1)
    ...
    #negative curve
    #
    points_neg_in_order = zeros((2,len(neg_points_stereographic[0,:])))

    #in polar coordinates
    #
    r_phi_negative_curve = zeros((len(neg_points_stereographic[0,:]),2))
    for ii in arange(len(neg_points_stereographic[0,:])):
        r_phi_negative_curve[ii,0] = sqrt(neg_points_stereographic[0,ii]**2 + neg_points_stereographic[1,ii]**2 )
        r_phi_negative_curve[ii,1] = arctan2(neg_points_stereographic[0,ii],neg_points_stereographic[1,ii])% (2*pi)
        
    #find index with highest r
    largest_r_idx = argmax(r_phi_negative_curve[:,0])

    #check, if perhaps more values with same r - if so, take point with lowest phi
    other_idces = list(where(r_phi_negative_curve[:,0]==r_phi_negative_curve[largest_r_idx,0]))
    if len(other_idces) > 1:
        best_idx      = argmin(r_phi_negative_curve[other_idces,1])
    ...
    if keep_direction:
        ...
        for jj in arange(len(neg_points_stereographic[0,:])):
    ...
    angle_point_1 = (arctan2(points_neg_in_order[0,0],points_neg_in_order[1,0])%(2*pi))
    angle_point_2 = (arctan2(points_neg_in_order[0,1],points_neg_in_order[1,1])%(2*pi))
    angle_point_3 = (arctan2(points_neg_in_order[0,2],points_neg_in_order[1,2])%(2*pi))
    ...
    if abs( angle_diff_12) > abs( angle_diff_23):
        r_old = sqrt(points_neg_in_order[0,0]**2 + points_neg_in_order[1,0]**2)
        new_angle = (angle_point_2 - angle_diff_23)%(2*pi)
        points_neg_in_order[0,0] = r_old * sin(new_angle)
        points_neg_in_order[1,0] = r_old * cos(new_angle)
        print 'corrected position of first point of negative curve to (%.2f,%.2f)\n'%(points_pos_in_order[0,0],points_pos_in_order[1,0])
        
            
    # check, if curve closed !!!!!!
    r_last_point          = sqrt(points_neg_in_order[0,-1]**2 + points_neg_in_order[1,-1]**2   )
    dist_last_first_point = sqrt( (points_neg_in_order[0,-1] - points_neg_in_order[0,0])**2 + (points_neg_in_order[1,-1] - points_neg_in_order[1,0] )**2 )


    if dist_last_first_point > (2 - r_last_point):
        #add points on edge to polygon, if it is an open curve
        print 'negative curve not closed - closing over edge...'
        phi_end   = arctan2(points_neg_in_order[0,-1],points_neg_in_order[1,-1])% (2*pi)
        phi_start = arctan2(points_neg_in_order[0,0],points_neg_in_order[1,0])% (2*pi)
        ...
        if go_ccw:
            ...
            for kk in arange(n_edgepoints):
                current_phi = phi_end - kk*openangle/(n_edgepoints-1)
                points_neg_in_order.append([2*sin(current_phi), 2*cos(current_phi) ]   )
            points_neg_in_order = array(points_neg_in_order).transpose()
        else:
            points_neg_in_order = list(points_neg_in_order.transpose())
            for kk in arange(n_edgepoints):
                current_phi = phi_end + kk*openangle/(n_edgepoints-1)
    ...
    def smooth_curve(curve, points_per_degree=1):
        ...
        smoothed_array      = zeros((1,2))
        smoothed_array[0,:] = coord_array[0]

        #now in shape (n_points,2)
        for idx,val in enumerate(coord_array[:-1]):
            r1   = sqrt(val[0]**2 + val[1]**2)
            r2   = sqrt(coord_array[idx+1][0]**2 + coord_array[idx+1][1]**2   )
            phi1 = arctan2(val[0],val[1])
            phi2 = arctan2(coord_array[idx+1][0],coord_array[idx+1][1])

            phi2_larger      = sign( phi2 - phi1 )
            angle_smaller_pi = sign( pi - abs(phi2 - phi1) )
            ...
            openangle_deg = openangle/pi*180
            radius_diff   = r2 -r1  
            
            if openangle_deg > 1./points_per_degree:
    
                n_fillpoints = int(openangle_deg * points_per_degree) 
                fill_array = zeros((n_fillpoints,2))
                if go_cw:
                    angles = ((arange(n_fillpoints)+1)*openangle/(n_fillpoints+1) + phi1)%(2*pi)
                else:
                    angles = (phi1 - (arange(n_fillpoints)+1)*openangle/(n_fillpoints+1) )%(2*pi)
                radii    = (arange(n_fillpoints)+1)*radius_diff/(n_fillpoints+1) + r1

                fill_array[:,0] = radii * sin(angles)
                fill_array[:,1] = radii * cos(angles)

                smoothed_array = append(smoothed_array,fill_array,axis=0)

            smoothed_array = append(smoothed_array,[coord_array[idx+1]],axis=0)
    ...
    outer_circle_points      = zeros((2,len(phi)))
    outer_circle_points[0,:] = 2 * sin(phi)
    outer_circle_points[1,:] = 2 * cos(phi)
    ...
    for idx,val in enumerate(points_pos_in_order.transpose() ):
        old_radius = sqrt(val[0]**2+val[1]**2) 
        if old_radius > 1:
    ...
    for idx,val in enumerate(all_EV_stereographic.transpose()):
        r_ev = sqrt(val[0]**2 + val[1]**2)
        if r_ev <= 1:
            lo_visible_EV.append([val[0],val[1],idx])
    visible_EVs = array(lo_visible_EV)

    #--------------------------------------------------------------------------
    # plot projection to unit sphere ('US')
  
    plotfig = figure(99,figsize=(5,5) )
    plotfig.subplots_adjust(left=0, bottom=0, right=1, top=1)
    
    ax = plotfig.add_subplot(111, aspect='equal')
    #axis([-1.103,1.103,-1.103,1.103],'equal')
    #xlim(xmin=-1.103, xmax=1.103)
    #ylim(ymin=-1.103, ymax=1.103)
    ...
    if outfile:
        if format:
            plotfig.savefig(outfile, dpi=100, transparent=True, format=format)
        else:
            plotfig.savefig(outfile, dpi=100, transparent=True)
...
def runplot(a,b,c,d,e,f, outfile=None, format=None):
    m_in = [a,b,c,d,e,f]
    M  = matrix(array([m_in[0],m_in[3],m_in[4],m_in[3],m_in[1],m_in[5],m_in[4],m_in[5],m_in[2] ]).reshape(3,3))
                          
    aa = plot_bb(M,outfile=outfile,format=format)
