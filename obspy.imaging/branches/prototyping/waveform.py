...
def plotWaveform(stream_object, outfile=None, format=None,
    ...
    for id in ids:
        ...
        for tr in stream_object:
            if tr.id == id:
                ...
                tr_ref = copy(tr)
                # XXX: trim does nothing if times are outside 
                if starttime >= tr_ref.stats.endtime or \
                        endtime <= tr_ref.stats.starttime:
                    continue
                tr_ref.trim(starttime, endtime)
                if tr_ref.data.size:
    ...
    # Default width.
    if not size:
        size = (800, len(stream_new)*250)

    # Find number of points
    fig = _setupPlot(size, dpi, starttime, endtime)

    for _i, tr in enumerat(stream_new):
        ax = fig.add_subplot(len(stream_new), 1, _i)
        max_freq = max([trmini.stats.sampling_rate for trmini in tr])
        if (endtime - starttime) * max_freq > 400000:
            _minmaxApproach()
        else:
            plotting()
...
def _setupPlot(size, dpi, starttime, endtime):
    ...
    fig = plt.figure(num=None, figsize=(float(size[0]) / dpi,
                     float(size[1]) / dpi))
    pattern = '%Y-%m-%d %H:%M:%S'
    suptitle = '%s  -  %s' % (starttime.strftime(pattern),
                              endtime.strftime(pattern))
    fig.suptitle(suptitle, x=0.02, y=0.96, fontsize='small',
