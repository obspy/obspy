from Tkinter import *
from matplotlib import figure, use as matplotlibuse
matplotlibuse('TkAgg')
...
def main():
    ...
    def changeLocationList(*args, **kwargs):
        ...
        for location in sorted_locations:
            ...
            if len(sorted_locations) == 1 and location == 'NOT AVAILABLE':
                location_box.itemconfig(0, fg = '#DCDCDC',
    ...
    network_container = LabelFrame(root, text = 'Network')
    network_container.grid(row=1, column=0)
    ...
    station_container = LabelFrame(root, text = 'Station')
    station_container.grid(row=1, column=1)
    station_box = Listbox(station_container)
    station_box.grid()
    location_container = LabelFrame(root, text = 'Location')
    location_container.grid(row=1, column=2)
    location_box = Listbox(location_container)
    location_box.grid()
    channel_container = LabelFrame(root, text = 'Channel')
    channel_container.grid(row=1, column=3)
    channel_box = Listbox(channel_container)
    channel_box.grid()
    selected_container = LabelFrame(root, text = 'Selected Channels')
    selected_container.grid(row=1, column=4)
    ...
    time_container.grid(row=2, column = 0, columnspan = 2, pady = 4,
                        padx = 8, sticky = W)
    ...
    info_label.grid(row=2, column = 2, columnspan = 2, sticky = E)
    ...
    colorcanvas.grid(row = 4, column = 0)
    
    status_bar = Label(root, height = 1, text = '')
    status_bar.grid(row = 4, column = 1, columnspan=2)
    ...
    button_frame2.grid(row=4, column = 3, columnspan = 2,
