...
def bwith(data,fs,smoothie,fk):
    ...
    Computes the bandwith of the given data which can be windowed or not.
    The bandwith corresponds to the level where the power of the spectrum is
    half its maximum value. It is determined as the level of 1/sqrt(2) times the
    maximum Fourier amplitude.

    If data are windowd the bandwith of each window is returned.
    ...
    if (np.size(data.shape)>1):
        ...
        for row in f_sm:
            ...
            [mdist_ind,mindist] = min(enumerate(minfc),key=itemgetter(1))
...
def logbankm(p,n,fs,w):
    ...
    fh = np.floor(fs/2)
    lr = np.log((fh)/(fl))/(p+1)
    bl = n*((fl)*np.exp(np.array([0,1,p,p+1])*float(lr)) /float(fs))
    b2 = np.ceil(bl[1])
    b3 = np.floor(bl[2])
    b1=np.floor(bl[0])+1
