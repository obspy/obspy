...
def gps2DistAzimuth(lat1,lon1,lat2,lon2) :
    ...
    # Iterate the following equations, until there is no significant change in dlon
    while ( last_dlon < -3000000.0 or dlon != 0 and abs( (last_dlon - dlon)/dlon) > 1.0e-9 ) :

        sqr_sin_sigma = pow( cos(U2) * sin(dlon), 2) + pow( (cos(U1) * sin(U2) - sin(U1) *  cos(U2) * cos(dlon) ), 2 )
        ...
        last_dlon = dlon
        dlon = omega + (1-C) * f * sin(alpha) * (sigma + C * sin(sigma) * (Cos2sigma_m + C * cos(sigma) * (-1 + 2 * pow(Cos2sigma_m, 2) )))
        ...
        alpha12 = atan2( (cos(U2) * sin(dlon)), (cos(U1) * sin(U2) - sin(U1) * cos(U2) * cos(dlon)))
        alpha21 = atan2( (cos(U1) * sin(dlon)), (-sin(U1) * cos(U2) + cos(U1) * sin(U2) * cos(dlon)))
