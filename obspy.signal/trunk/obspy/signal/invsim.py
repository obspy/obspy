...
def pazToFreqResp2(poles, zeros, scale_fac, t_samp, nfft):
    """
    Convert Poles and Zeros (PAZ) to frequency response. Fast version which
...
def pazToFreqResp3(poles, zeros, scale_fac, t_samp, nfft):
    """
    ...
    for i in xrange(0, nfft / 2 + 1):
        s = complex(0.0, i * 2 * N.pi * delta_f)
        num = 1.0 + 0.0j
        for ii in xrange(nzero):
            num *= (s - zeros[ii])
...
def simFilt(tr2, tsa, fc0, fc1, fc2, h0, h1, h2, do_int):
    ...
    if fc0 <= 0.0:
        ...
        oms0 = M.tan(tsa * N.pi * fc0)

    oms1 = M.tan(tsa * N.pi * fc1)
    oms2 = M.tan(tsa * N.pi * fc2)
