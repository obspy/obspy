...
def envelope(data):
    ...
    not. The envelope is determind by the absolute value of the analytic
...
def normEnvelope(data,fs,smoothie,fk):
    ...
    the instantaneous bandwith of the smoothed envelope is normalized by the
...
def centroid(data,fk):
    ...
    not. The centroid time is determind as the time in the processed
        ...
        for row in x[1]:
            ...
            for k in xrange(2,size(row)):
                ...
                if (t >= half):
                    ...
                    break
            i = i+1
        centroid_add = np.append(np.append([centroid[0]]*(size(fk)/2),centroid),
                              [centroid[size(centroid)-1]]*(size(fk)/2))
...
def instFreq(data,fs,fk):
    ...
    windowed or not. The instantaneous frequency is determind by the time
...
def instBwith(data,fs,fk):
    ...
    Computes the instantaneous bandwith of the given data which can be
    windowed or not. The instantaneous bandwith is determind by the time
    derivative of the envelope normalized by the envelope of the input data.
    
    @param data: Data to determine instantaneous bandwith of, type
        numpy.ndarray.
    @params fs: Sampling frequency.
    @params fk: Filter coefficients for computing time derivative.
    @return sigma: Instantaneous bandwith of input data.
    @return dsigma: Time derivative of instantaneous bandwith.
    """
    x = envelope(data)
    if (size(x[1].shape)>1):
