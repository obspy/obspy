...
def envelope(data):
	...
	A_cpx = zeros((data.shape),dtype='complex64')
	A_real = zeros((data.shape),dtype='float64')
...
def normEnvelope(data,fs,smoothie,fk):
	...
	if (size(x[1].shape)>1):
		i = 0
		Anorm = zeros(x[1].shape[0],dtype='float64')
		for row in x[1]:
			A_win_smooth = util.smooth(row,smoothie)
			# Differentiation of original signal, dA/dt
			A_win_add = append(append([row[0]]*(size(fk)/2),row),[row[size(row)-1]]*(size(fk)/2))
			...
			Anorm[i] = ( (exp(mean(t_))) -1 )*100
			i = i+1
		#Anorm = util.smooth(Anorm,smoothie)
		Anorm_add = append(append([Anorm[0]]*(size(fk)/2),Anorm),[Anorm[size(Anorm)-1]]*(size(fk)/2))
		dAnorm = signal.lfilter(fk,1,Anorm_add)
		dAnorm = dAnorm[size(fk)/2:(size(dAnorm)-size(fk)/2)]
		return Anorm, dAnorm
	else:
		Anorm = zeros(1,dtype='float64')
		A_win_smooth = util.smooth(x[1],smoothie)
		# Differentiation of original signal, dA/dt
		A_win_add = append(append([x[1][0]]*(size(fk)/2),x[1]),[x[1][size(x[1])-1]]*(size(fk)/2))
		t = signal.lfilter(fk,1,A_win_add)
		t = t[size(fk)/2:(size(t)-size(fk)/2)]
		for k in xrange(0,size(A_win_smooth)):
        		if (A_win_smooth[k] < 1):
...
def centroid(data,fk):
	...
	x = envelope(data)
	if (size(x[1].shape)>1):
		centroid = zeros(x[1].shape[0],dtype='float64')
		i = 0
		for row in x[1]:
			# Integral within window
			half = 0.5*sum(row)
			...
			for k in xrange(2,size(row)):
				...
				if (t >= half):
					...
					break
			i = i+1
		centroid_add = append(append([centroid[0]]*(size(fk)/2),centroid),[centroid[size(centroid)-1]]*(size(fk)/2))
		...
		centroid = zeros(1,dtype='float64')
		# Integral within window
		half = 0.5*sum(x[1])
		# Estimate energy centroid 
...
def instFreq(data,fs,fk):
	...
	x = envelope(data)
	if (size(x[0].shape)>1):
		omega = zeros(x[0].shape[0],dtype='float64')
		i = 0
		for row in x[0]:
			f = real(row)
			h = imag(row)
			f_add = append(append([f[0]]*(size(fk)/2),f),[f[size(f)-1]]*(size(fk)/2))
			fd = signal.lfilter(fk,1,f_add)
			fd = fd[size(fk)/2:(size(fd)-size(fk)/2)]
			h_add = append(append([h[0]]*(size(fk)/2),h),[h[size(h)-1]]*(size(fk)/2))
		...
		omega = zeros(size(x[0]),dtype='float64')
		f = real(x[0])
		h = imag(x[0])
		f_add = append(append([f[0]]*(size(fk)/2),f),[f[size(f)-1]]*(size(fk)/2))
		fd = signal.lfilter(fk,1,f_add)
	...
	omega_add = append(append([omega[0]]*(size(fk)/2),omega),[omega[size(omega)-1]]*(size(fk)/2))
...
def instBwith(data,fs,fk):
	...
	x = envelope(data)
	if (size(x[1].shape)>1):
		sigma = zeros(x[1].shape[0],dtype='float64')
		i = 0
		for row in x[1]:
			A_win_add = append(append([row[0]]*(size(fk)/2),row),[row[size(row)-1]]*(size(fk)/2))
			t = signal.lfilter(fk,1,A_win_add)
		...
		sigma = zeros(size(x[0]),dtype='float64')
		A_win_add = append(append([x[1][0]]*(size(fk)/2),x[1]),[x[1][size(x[1])-1]]*(size(fk)/2))
		t = signal.lfilter(fk,1,A_win_add)
		t = t[size(fk)/2:(size(t)-size(fk)/2)]
		sigma = abs((t*fs)/(x[1]*2*pi))
	sigma_add = append(append([sigma[0]]*(size(fk)/2),sigma),[sigma[size(sigma)-1]]*(size(fk)/2))
