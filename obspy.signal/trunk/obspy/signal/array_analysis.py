...
def sonic(stream, win_len, win_frac, sll_x, slm_x, sll_y, slm_y, sl_s,
    ...
    nfft = nextpow2(nsamp)
    wave = np.fft.fftpack.fftpack.rffti(nfft)
    cosine = cosTaper(nsamp, 0.2).astype('float64')
    newstart = stime
    offset = 0
    while eotr:
        try:
...
def bbfk(spoint, offset, trace, ntrace, stat_tshift_table, flow, fhigh,
         digfreq, nsamp, nstat, prewhiten, grdpts_x, grdpts_y, wave, nfft,
         cosine):
    """
    ...
    :double* wave: The plan generated by np.fft.fftpack.fftpack.rffti(nfft),
               for speed purposes
    :int nfft: Number of points to use for fft
    :double *cosine: Cosine taper as numpy.ndarray, as generated by
                 array obspy.signal.invsim.cosTaper(nsamp, 0.2)
