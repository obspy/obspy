...
class ObsPyck(QtGui.QMainWindow):
    ...
    def on_qToolButton_deleteEvent_clicked(self, dummy_int):
        ...
        user = event.get('user')
        # XXX XXX OLD GTK, UPDATE TO QT
        # XXX dialog = gtk.MessageDialog(self.win, gtk.DIALOG_MODAL,
        # XXX                            gtk.MESSAGE_INFO, gtk.BUTTONS_YES_NO)
        # XXX msg = "Delete event from database?\n\n"
        # XXX msg += "<tt><b>%s</b> (account: %s, user: %s)</tt>" % (resource_name,
        # XXX                                                        account, user)
        # XXX dialog.set_markup(msg)
        # XXX dialog.set_title("Delete?")
        # XXX response = dialog.run()
        # XXX dialog.destroy()
        # XXX if response == gtk.RESPONSE_YES:
        # XXX     self.deleteEventInSeishub(resource_name)
        # XXX     self.on_qToolButton_updateEventList_clicked(event)
