...
if sys.platform=='win32':
    clibmseed = cdll.libmseed
    clib = cdll.msvcrt
    #Windows DLL not tested
else:
    clibmseed = C.CDLL('./libmseed.so')
    clib = C.CDLL('libc.so.6')

#imports all libmseed data structures
from libmseedstructures import *
...
class libmseed(object):   
   ...
   def __getattr__(self, name):
       ...
       currently only works with the first record of the Mseedfile and the test.mseed file
       """
   ...
   def ms_printtracelist(self,filename,timeformat= 0,details = 0, gaps=0):
       """mst_printtracelist prints a formatted list of the MSTrace segments 
