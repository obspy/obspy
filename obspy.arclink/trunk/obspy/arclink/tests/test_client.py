...
#        self.assertEquals(results, {})
    ...
    def test_getInventory(self):
        ...
        """
        expected_results = AttribDict({
            'remark': '', 'code': 'BW', 'end': None,
            'description': 'BayernNetz',
            'stations': AttribDict({
                'RJOB': AttribDict({
                    'remark': '', 'code': 'RJOB', 'elevation': 860.0,
                    'description': 'Jochberg, Bavaria',
                    'start': UTCDateTime(2007, 6, 4, 0, 0),
                    'restricted': False, 'archive_net': '',
                    'longitude': 12.795714, 'affiliation': '', 'depth': 0.0,
                    'place': '', 'country': '', 'latitude': 47.737166999999999,
                    'end': None})}),
        ...
        dt = UTCDateTime(2010, 1, 1)
        #1 - BW network via erde.geophysik.uni-muenchen.de:18001
        client = Client(host="erde.geophysik.uni-muenchen.de", port=18001, debug=True)
        results = client.getInventory('BW', 'RJOB', dt, dt + 1)
        self.assertEquals(len(results), 1)
        self.assertTrue('BW' in results)
        self.assertEquals(results.BW, expected_results)
        #2 - BW network via webdc:18001
        client = Client(host="webdc.eu", port=18001)
        results = client.getInventory('BW', 'RJOB', dt, dt + 1)
        self.assertEquals(len(results), 1)
        self.assertTrue('BW' in results)
        print results.BW
        expected_results.stations.RJOB.start = UTCDateTime(2006, 7, 18, 0, 0)
